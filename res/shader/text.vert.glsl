...
void main() {
    ...
    position = vec2(position.x / (screen_size.x / 2.0), -position.y / (screen_size.y / 2.0)) - vec2(1.0, -1.0); 
    ...
    frag_texture_coordinate = vec2(frag_texture_coordinate.x / texture_size.x, frag_texture_coordinate.y / texture_size.y);
}