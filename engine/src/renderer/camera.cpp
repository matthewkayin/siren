...
glm::vec3 siren::Camera::get_position() const {
    return position;
}

void siren::Camera::set_position(glm::vec3 value) {
    position = value;
    dirty = true;
}

glm::vec3 siren::Camera::get_forward() const {
    return forward;
}

glm::vec3 siren::Camera::get_up() const {
    return up;
}

glm::vec3 siren::Camera::get_right() const {
    return glm::cross(forward, up);
}

void siren::Camera::look_at(glm::vec3 point) {
...
glm::mat4 siren::Camera::get_view_matrix() {
    if (dirty) {
